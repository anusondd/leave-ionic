...
import { Component, OnInit, ViewChild, ViewChildren, QueryList, AfterViewChecked, AfterViewInit, OnChanges, SimpleChanges } from '@angular/core';
...
export class ApproveTypePage implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(){
    this.approveTypeLOVs.changes.subscribe((r) => { this.onAddApproverChange(); });
  ...
  addApprovers(approveLevelLoop) {
    ...
    this.indexForAddEmp = this.indexForAddEmpLevel[this.approveLevel.length - 1];
    //this.addLOVOption();
    //console.log("approveTypeLOVs :",this.approveTypeLOVs);
 ...
 this.approveTypeService.editApproveType(approveType).then(result => {
  ...
  onRowSelectEmployee(event, approvers) {
